...
22. Growth strategy must start from the primary business outcome and a supported binding constraint, not a generic channel checklist. Do not force fixed planning horizons, priority counts, funnel models, channel mixes, budget ratios, or benchmark-based opportunities. Preserve specialist ownership, explicit non-priorities, capacity limits, decision gates, and the boundary that paid-media scaling remains under `$optimization-scaling`.

...
23. Structured marketing/business data must preserve the `DATA-CONTRACTS.md` envelope: source/provenance, represented period, timezone, currency, grain, stable key, conversion/event meaning, attribution/revenue/profit basis, field lineage, missingness, and known limitations. Preserve raw and normalized layers separately; never duplicate money/outcomes through incompatible joins, turn missing rows into zero without support, force different attribution systems to equality, or treat a dataset as universally valid because it is valid for one scoped decision.

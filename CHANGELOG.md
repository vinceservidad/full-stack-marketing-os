...
- `$growth-strategy`, thirtieth governed skill, adds the native cross-functional strategy and marketing-planning layer. It owns business-objective framing, growth-system/constraint mapping, opportunity portfolio, strategic prioritization, sequencing, explicit non-priorities, and review/rebalancing without taking over specialist decisions, paid-media scaling, recurring operations, or live implementation.
- Adds three Growth Strategy references for system/constraint diagnosis, evidence-led opportunity prioritization/portfolio design, and planning horizons/review. [`templates/strategy-template.md`](templates/strategy-template.md) is upgraded from a shallow generic checklist into the canonical growth-strategy plan and reassigned from `$marketing-router` to `$growth-strategy`.
- The system explicitly rejects universal 30/60/90 or 90-day roadmaps, 70/20/10 or fixed experiment-budget allocations, mandatory channel counts, ROAS-only portfolio decisions, and roadmap approval as live-change authorization. `$optimization-scaling` still owns paid-media readiness/marginal expansion; `$marketing-operations` owns recurring execution loops; Marketing Context stores only current decision-relevant strategy state.
- Sixty Growth Strategy behavioral evaluations plus completed review cover missing objectives/baselines, default roadmaps and budget splits, channel quotas, competitor copying, ROAS-only allocation, verified defects and measurement blockers, false bottlenecks, seasonality, activation/retention relevance, offer/pricing/capacity dependencies, backlog masquerading as strategy, false-precision scoring, ownerless initiatives, scaling boundaries, approval/state truth, contradictory learning, stale plans, lag/cohort maturity, parallel-test contamination, external benchmarks, routing boundaries, non-marketing constraints, and insufficient evidence.

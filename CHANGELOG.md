...
- Ninety Growth Strategy behavioral evaluations plus completed review cover channel-first plans, framework quotas, constraint misdiagnosis, measurement/economics/capacity blockers, opportunity construction, false scoring, specialist ownership, paid-media scaling boundaries, forecasting, context staleness, causal overclaiming, planning horizons, exact status, operations/reporting handoffs, non-priorities, reversibility, dependencies, null/contradictory learning, and live-action authorization.
...
### Added — tooling, distribution, and executable evaluation

- **Distribution.** `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` make the repository installable as a Claude Code plugin. `plugin.json` points `skills` at `./.agents/skills/`, so the canonical layer does not move; `agents`, `commands`, and `workflows` are explicitly emptied so the root `agents/` and `workflows/` documentation directories are not auto-discovered as plugin components.
- **`scripts/eval.py`** — an executable evaluation harness over the existing corpus, standard library only, no new dependency. `--static` runs in CI and verifies that every case parses, carries a pass criterion, and belongs to a registered suite naming a skill that exists; that no knowledge layer makes a claim the system's own rules forbid; and that the skill names written in `agents/` and `evaluations/` resolve. `--live` scores cases against a model and writes a dated scorecard to `tests/RESULTS.md`; it requires `ANTHROPIC_API_KEY` and skips cleanly without one. No case file was rewritten — the parser handles all three authoring shapes already in use.
- **`tests/evaluations/suites.json`** registers all 41 suites and 873 cases, asserting each suite's parsed case count so prose and manifest cannot drift apart silently. Five pre-v0.3 suites are recorded as having no human review record rather than being quietly exempted.
- **`evaluations/routing-tests.md`** rebuilt as an executable 16-case suite covering contestable routing boundaries — the paid-hook versus lifecycle-copy split, single-channel versus cross-channel reporting ownership, YouTube format versus the shared Google Ads bidding layer, an escalation from organic social to public relations, and an uncovered dashboard request that must be declared as a gap rather than absorbed by an adjacent skill.
...
- **`scripts/build-gpt-knowledge.py`** generates `gpt-knowledge/pack/` — 18 Custom GPT knowledge files compiled from all 30 governed skills, their references, and the root contracts, plus generated GPT instructions and a manifest. `--check` runs in CI and fails on two things: a hand-edit of generated output, and **a governed skill with no export bundle**, so a new skill cannot ship without reaching the GPT layer.
- `.gitignore`, `docs/README.md`, `agents/README.md`, `docs/archive/launch/README.md`, and `docs/archive/gpt-knowledge-v1/README.md`.

### Fixed

- **`scripts/install-skills.sh` failed on Linux.** The link-depth rewrite used `sed -i ''`, which is BSD/macOS syntax; GNU sed reads `''` as a filename, exits 2, and `set -euo pipefail` aborted the install. The rewrite now runs through `python3`, which the repository already requires, and behaves identically on both platforms. The macOS-only bug shipped undetected because the installer had no CI coverage; it now has some.
- `scripts/validate-skill-architecture.sh` checked installed-runtime drift only under `~/.codex/skills`. It now checks every runtime the installer can target and labels each finding by runtime.
- `ARCHITECTURE.md` claimed `gpt-knowledge/` contains material "including Search Engine Optimization, copywriting, and reporting — for which no governed specialist exists." All three have been governed since v1.7–v1.9. Replaced with an accurate statement of why a derived export can diverge from the canonical layer.
- `agents/router.md` and the six role documents routed to "Analytics Skill", "Content Framework", and "Paid Media Specialist" — names absent from `CAPABILITY-REGISTRY.md`. They now map each human role to the governed skills that own its work and defer to `$marketing-router`. `scripts/eval.py --static` enforces that these names keep resolving, extending to `agents/` and `evaluations/` the rule the architecture validator already applied to the router.

### Changed

- **The Custom GPT export layer is now generated rather than hand-written.** It had drifted to roughly 1,100 words against 40,000 in `.agents/skills/`, with nine governed skills — `$affiliate-marketing`, `$influencer-marketing`, `$lifecycle-marketing`, `$linkedin-ads`, `$organic-social`, `$programmatic`, `$public-relations`, `$tiktok-ads`, `$youtube-ads` — having no export representation at all. A GPT loaded with it was not running this operating system: it had none of the evidence-state discipline, the authorization boundary, or the scaling gates. The generated pack is 77,037 words and covers every governed skill by construction. The nine superseded files are archived at `docs/archive/gpt-knowledge-v1/`; `vince-style.md` and `humanizer-rules.md` remain hand-maintained, being voice material derived from no skill.

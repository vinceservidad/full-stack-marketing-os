...
- **`tests/evaluations/suites.json`** registers all 28 suites and 415 cases, asserting each suite's parsed case count so prose and manifest cannot drift apart silently. Five pre-v0.3 suites are recorded as having no human review record rather than being quietly exempted.

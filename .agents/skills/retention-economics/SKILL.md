...
description: Model customer lifetime value, payback period, cohort retention, and repeat-purchase or renewal economics using realized revenue and margin; not for single-period efficiency metrics, platform ROAS, defining activation, or setting pricing architecture.
---
...
5. Treat material pricing, package, payment-model, acquisition-offer, or activation-definition/journey changes as possible cohort boundaries. Compare pre-change and post-change cohorts separately before pooling them.
...
7. Compute payback period against the same profit level used for cost of acquisition; state whether payback is measured in revenue or contribution.
8. Separate new-customer economics from returning-customer economics; do not blend acquisition cost into a blended lifetime figure that then hides unprofitable acquisition.
9. State the confidence interval or the immaturity discount on any predictive figure, and what evidence would tighten it.
...
- Do not average retention or lifetime value across cohorts of materially different age, channel, acquisition offer, activation journey/definition, price, package, or payment model without stating that the blend can mask a declining or improving trend.
...
- Do not use predictive lifetime value alone to authorize an activation, pricing, or scaling decision. `$activation` owns first-value journey decisions; `$pricing-monetization` owns pricing structure; `$optimization-scaling` owns scaling and applies its own proof standard and marginal-economics gates.
...
Cohort economics: cohort definition and window; activation definition/state where relevant; pricing/package state where relevant; historical or predictive label; profit level; retention or renewal curve with maturity state; lifetime value at stated horizons; payback period; new versus returning economics; confidence or immaturity discount; comparison to acquisition cost; exact status.

## QA

Confirm the cohort definition and window are stated, material activation/pricing/package changes are not silently pooled, activated-versus-nonactivated comparisons are not treated as causal by default, historical and predictive figures are never blended, the profit level is named and consistent with acquisition cost, curves are not averaged across incompatible cohorts, extrapolation past observed maturity is disclosed, and no figure here alone authorizes an activation, pricing, or scaling change.

...
- Other unsupported channels — affiliate, influencer, organic social, programmatic, public relations: the router owns the response and declares the gap. Search Engine Optimization is governed by `$seo`; email and lifecycle program strategy is governed by `$lifecycle-marketing`; YouTube video advertising is governed by `$youtube-ads`; TikTok advertising is governed by `$tiktok-ads`; LinkedIn advertising is governed by `$linkedin-ads`. Do not still describe any of these five as unsupported.

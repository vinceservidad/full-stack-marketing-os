...
- Other unsupported channels — TikTok, LinkedIn, affiliate, influencer, organic social, programmatic, public relations: the router owns the response and declares the gap. Search Engine Optimization is governed by `$seo`; email and lifecycle program strategy is governed by `$lifecycle-marketing`; YouTube video advertising is governed by `$youtube-ads`. Do not still describe any of these three as unsupported.

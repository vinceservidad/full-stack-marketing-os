...
- Other unsupported channels — programmatic, public relations: the router owns the response and declares the gap. Search Engine Optimization is governed by `$seo`; email and lifecycle program strategy is governed by `$lifecycle-marketing`; YouTube video advertising is governed by `$youtube-ads`; TikTok advertising is governed by `$tiktok-ads`; LinkedIn advertising is governed by `$linkedin-ads`; influencer and creator partnerships are governed by `$influencer-marketing`; affiliate and partner programs are governed by `$affiliate-marketing`; organic social content is governed by `$organic-social`. Do not still describe any of these eight as unsupported.

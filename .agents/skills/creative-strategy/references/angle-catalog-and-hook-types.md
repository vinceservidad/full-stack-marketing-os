...
A checklist of common angle categories and hook openings, for scanning against a chosen strategic direction — distinct from [Strategic framing and angle archetypes](strategic-framing-and-angle-archetypes.md), which covers generative techniques for *finding* a novel angle. This reference is a coverage tool: after method step 1's evidence-gathering, use it to check whether an obvious angle category was overlooked, not to pick one by default.

Classify as a catalog/checklist per [`KNOWLEDGE-TAXONOMY.md`](../../../../KNOWLEDGE-TAXONOMY.md). A category name is not evidence a given execution of it will work for this audience; every angle drawn from this catalog still requires the skill's existing hypothesis statement and test matrix.

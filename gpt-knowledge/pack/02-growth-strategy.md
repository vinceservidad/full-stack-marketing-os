...
Primary knowledge type: strategy. Use `KNOWLEDGE-TAXONOMY.md` to label supporting frameworks, methodologies, hypotheses, tactics, templates, and evidence correctly.
...
3. **Identify the current limiting condition(s).** Use Growth constraint and opportunity diagnosis. Separate symptoms from constraints. Name a primary/binding constraint only when the evidence supports one; otherwise preserve a constraint set, independent constraints, or `not yet identified` rather than forcing false singularity.
...
5. **Prioritize strategic bets.** Use Portfolio prioritization and sequencing. Compare commercial impact, evidence strength, mechanism confidence, effort, reversibility, time to learn, dependencies, capacity, and opportunity cost without hiding uncertainty in fake precision.
...
9. **Define review and adaptation.** Use Plan governance and review. Set context-appropriate review triggers/horizons, decision gates, and `continue`, `hold`, `kill`, `revise`, `defer`, or `route to scaling` outcomes. Material opportunity-cost changes are valid review triggers even when the original initiative has not failed.
...
- strategy-template.md — canonical integrated growth-strategy and marketing-plan record.
...
### Reference: growth constraint and opportunity ($growth-strategy)
...
### Reference: plan governance and review ($growth-strategy)
...
### Reference: portfolio prioritization and sequencing ($growth-strategy)

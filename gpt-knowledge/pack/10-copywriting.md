...
**Use when:** Write or evaluate email, lifecycle, website, sales-page, long-form, or brand copy using evidence-backed customer language and a structure matched to funnel stage; not for paid-ad hooks or landing-page conversion copy, which are owned elsewhere.

Classify each framework, structure, or heuristic with `KNOWLEDGE-TAXONOMY.md`. A copywriting structure (AIDA, PAS, FAB, BAB) is a pattern that organizes an argument — it is not evidence that the argument is true, and applying it correctly does not make an unsupported claim supportable.

Per `CAPABILITY-REGISTRY.md`, paid-ad hooks, angles, concepts, and static paid-creative text hierarchy stay owned by `$creative-strategy`; landing-page and product-page conversion copy stays owned by `$cro`. This skill owns what those do not: email and lifecycle sequences, website and sales-page copy, long-form content, and brand voice — the copywriting work with no other current owner.
...
4. Select a structure matched to the communication goal, not by default. See Structure selection.
...
### Reference: structure selection ($copywriting)
